...
bool Array<P_numtype, N_rank>::isStorageContiguous() const
{
    ...
    for (int i=0; i < N_rank; ++i)
    {
      ptrdiff_t stride = BZ_MATHFN_SCOPE(abs)(stride_[i]);
        if (stride == 1)
            haveUnitStride = true;

        ptrdiff_t vi = stride * length_[i];
...
void Array<P_numtype, N_rank>::reverseSelf(int rank)
{
    ...
    ptrdiff_t adjustment = static_cast<ptrdiff_t>(stride_[rank]) * (lbound(rank) + ubound(rank));
    ...
    int componentNumber, int numComponents) const
{
    ...
    TinyVector<ptrdiff_t, N_rank> stride2;
    for (int i=0; i < N_rank; ++i)
