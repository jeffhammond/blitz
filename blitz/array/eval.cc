    ...
    T_expr expr, T_update)
{
    ...
    BZ_DEBUG_MESSAGE("BZ_USE_FAST_READ_ARRAY_EXPR with commonStride");
	  ...
	  cout << "evaluateWithStackTraversal1 " << ubound << endl;

	  for (int i=0; i < ubound; i+=simdTypes<T_numtype>::vecWidth)
	    #pragma ivdep
	    T_update::update(*reinterpret_cast<typename simdTypes<T_numtype>::vecType*>(&data[i]), 
			     expr.fastRead_tv(i));
 #else
