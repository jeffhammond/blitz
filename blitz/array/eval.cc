    ...
    T_expr expr, T_update)
{
    ...
    ptrdiff_t commonStride = expr.suggestStride(firstRank);
...
#endif
        ptrdiff_t ubound = length(firstRank) * commonStride;
...
#else
                const ptrdiff_t t1 = i+1;
                const ptrdiff_t t2 = i+2;
                const ptrdiff_t t3 = i+3;
  ...
  #ifndef BZ_ARRAY_STACK_TRAVERSAL_UNROLL
            for (ptrdiff_t i=0; i != ubound; i += commonStride)
                T_update::update(data[i], expr.fastRead(i));
  #else
            ptrdiff_t n1 = (length(firstRank) & 3) * commonStride;

            ptrdiff_t i = 0;
            for (; i != n1; i += commonStride)
                T_update::update(data[i], expr.fastRead(i));

            ptrdiff_t strideInc = 4 * commonStride;
            for (; i != ubound; i += strideInc)
            {
                T_update::update(data[i], expr.fastRead(i));
                ptrdiff_t i2 = i + commonStride;
                T_update::update(data[i2], expr.fastRead(i2));
                ptrdiff_t i3 = i + 2 * commonStride;
                T_update::update(data[i3], expr.fastRead(i3));
                ptrdiff_t i4 = i + 3 * commonStride;
    ...
    T_expr expr, T_update)
{
     ...
     */
#ifdef BZ_ARRAY_EXPR_USE_COMMON_STRIDE
    ptrdiff_t commonStride = expr.suggestStride(maxRank);
    ...
    while (true) {
        ...
        if ((useUnitStride) || (useCommonStride))
        {
            ...
            ptrdiff_t ubound = lastLength * commonStride;
    ...
    T_expr expr, T_update)
{
...
#ifdef BZ_ARRAY_EXPR_USE_COMMON_STRIDE
    ptrdiff_t commonStride = expr.suggestStride(minorRank);
    ...
    ptrdiff_t commonMajorStride = expr.suggestStride(majorRank);
            ...
            if (bj + tileWidth <= maxj)
            {
                ...
                if ((useUnitStride) && (haveCommonMajorStride))
                {
                    ...
                    for (int i=bi; i < ni; ++i)
                    {
                        ...
                        ptrdiff_t t1 = offset+1;
                        ptrdiff_t t2 = offset+2;
